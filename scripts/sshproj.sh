...
curl -s -X POST --header "Content-Type:application/json" --header "Authorization: Bearer $token" -d $query https://data.hasura.io/v1/query | jq -r '.[0].ssh_keys' | base64 --decode | jq -r '.private' > ~/projssh/$project.private.pem
...
set -x
ssh -p 2022 -i ~/projssh/$project.private.pem core@$project.hasura-app.io
